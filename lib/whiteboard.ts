...
export function initializeWhiteboard(container: HTMLDivElement) {
...
export function initializeWindowEvents(canvas: fabric.Canvas) {
...
export function loadData(canvas: fabric.Canvas, data: string) {
...
export function switchTool(canvas: fabric.Canvas, tool: Tool, color: string) {
    ...
    case 'Circle':
      {
        ...
        canvas.on('mouse:move', (opt) => {
          if (isDragging && circle) {
            ...
            circle.set({
              radius:
                Math.abs(lastPosX - pointer.x) > Math.abs(lastPosY - pointer.y)
